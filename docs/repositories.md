...
**Lockfiles read.** npm (`package-lock.json`, `npm-shrinkwrap.json`), pnpm (`pnpm-lock.yaml`), Yarn (`yarn.lock`, both the classic format and Berry's), Bun (`bun.lock`), Python (`poetry.lock`, `uv.lock`, `pdm.lock`, `Pipfile.lock`), Composer (`composer.lock`), Swift (`Package.resolved`), CocoaPods (`Podfile.lock`), Dart (`pubspec.lock`) and Helm (`Chart.lock`). Transitive entries count: most of an npm tree arrives transitively and a CVE does not care how a package got into the build.
