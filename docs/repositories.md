...
Manifests parsed include npm, Python — `requirements.txt`, `Pipfile`, every lockfile below, and `pyproject.toml` **read for every tool that declares dependencies in it: PEP 621 `[project]`, extras, PEP 735 `[dependency-groups]` as `uv` writes them, and the Poetry, PDM and Hatch tables** — Go, Cargo, Maven, Gradle — build files **and `gradle/libs.versions.toml` version catalogs, where a modern Android or Kotlin Multiplatform project actually declares everything** — Scala (`build.sbt`) and Ivy (`ivy.xml`), Elixir and Erlang (`mix.exs`, `rebar.config`), C and C++ (Conan's `conanfile.txt`/`conanfile.py` and vcpkg's `vcpkg.json`), Haskell (`*.cabal` and `stack.yaml`), OCaml (`opam`, `*.opam` and `dune-project`), Conda (`environment.yml`), Zig (`build.zig.zon`), Lua (`*.rockspec`), RubyGems (`Gemfile`, `fastlane/Pluginfile` and a gem's own `*.gemspec`, which is what a library repository has where an application has a Gemfile), NuGet — `.csproj`/`.fsproj`/`.vbproj` and **Paket's `paket.dependencies`, which replaces NuGet restore entirely, so a Paket repository has no useful PackageReference version anywhere** — Composer, Terraform, Swift (`Package.resolved`), CocoaPods (`Podfile.lock`), Dart and Flutter (`pubspec.yaml` and `pubspec.lock`), Helm charts (`Chart.yaml` and `Chart.lock`), Dockerfiles and **GitHub Actions workflows** — CI pulls third-party actions and container images by tag, and a tag nobody upgrades on purpose is exactly where an old vulnerable dependency hides.

**Conda is its own ecosystem, not pip.** Conda resolves from its own channels and the two disagree about names: `pytorch` on conda-forge is `torch` on PyPI, `opencv` is `opencv-python`. Filing a Conda dependency as pip would send every version lookup and every CVE correlation to the wrong package, which is worse than not reading the file — a wrong answer reads as an answer. The nested `pip:` list inside an `environment.yml` is the exception: those really are PyPI names, and they are filed as pip. A `conda-lock.yml` states which manager installed each package, so the split comes straight off the file. No version lookup: anaconda.org answers per channel, and a repository pinning a channel other than conda-forge would get an answer about a different build.

**vcpkg names dependencies, it does not version them.** A vcpkg dependency usually has no version in the manifest at all — the registry's baseline commit decides which version is installed. So a vcpkg row is a name with the version reported unknown, and that is the correct answer rather than a gap. `version>=` is stored when present, as a floor. `vcpkg-configuration.json` is not a lock file despite being described as one in places: it holds registry and baseline configuration and no dependencies.

**A conanfile is a program, and is read partially.** `self.requires("zlib/1.3.1")` and a `requires = [...]` attribute are read; a requirement computed in a loop is not. rippled's own conanfile has both forms plus conditionals, and a partial read that says what it read beats executing a build script. `CMakeLists.txt` is deliberately out of scope: `FetchContent` pins a git tag, which is a source reference and not a published version.

**`build.sbt` is read partially, on purpose.** It is a Scala program, and real ones are written with variables: `typelevel/cats` declares `"org.scalacheck" %%% "scalacheck" % scalaCheckVersion` and sets `scalaVersion := Scala213`. Only quoted arguments are read, so most dependencies in a real build file are named with the version reported unknown — which is the honest answer rather than storing the word `scalaCheckVersion`. `%%` appends the Scala binary version to the artifact name, and that suffix is only added when `scalaVersion` is a literal; guessing would name an artifact that may not exist. A project that keeps its dependencies in `project/Dependencies.scala` is not read at all: `scalatest`'s `build.sbt` delegates everything and declares nothing, and zero is the right answer for it.
